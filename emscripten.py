...
def update_settings_glue(metadata):
  ...
  shared.Settings.DEFAULT_LIBRARY_FUNCS_TO_INCLUDE = sorted(
    set(shared.Settings.DEFAULT_LIBRARY_FUNCS_TO_INCLUDE + map(shared.JS.to_nice_ident, metadata['declares'])).difference(
...
def create_mftCall_funcs(function_table_data):
  ...
  for sig, table in function_table_data.items():
    ...
    coerced_params = ','.join([shared.JS.make_coercion('ptr', 'i')] + [shared.JS.make_coercion('p%d', unfloat(sig[p + 1])) % p for p in range(len(sig))])
